...
Returns:
    ...
    Errors:
        ...
        try:
            r = requests.post(URLs.URL_UPLOAD_CLOSE_FILE, data=payload)
    ...
    def create_file(self, session_id, folder_id, file_name, file_size, access_folder_id=""):
        """Method:
    Post
...
Notes:
    Create file should be called before upload."""
        ...
        try:
            r = requests.post(URLs.URL_UPLOAD_CREATE_OPEN, data=payload)
            if r.status_code is 200:
                return r.json()
...
Notes:
    Open file upload should be called before file upload."""
        ...
        try:
            r = requests.post(URLs.URL_UPLOAD_OPEN, data=payload)
            if r.status_code is 200:
                return r.json()
